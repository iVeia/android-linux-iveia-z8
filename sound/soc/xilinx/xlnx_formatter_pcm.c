					...
					struct snd_pcm_hw_params *params)
{
	...
	val = readl(stream_data->mmio + XLNX_AUD_CTRL);
	val &= ~(0x3 << AUD_CTRL_DATA_WIDTH_SHIFT);
	bits_per_sample = params_width(params);
	...
	val &= ~(0xf << AUD_CTRL_ACTIVE_CH_SHIFT);
	...
	if (aud_drv_data->nodes[XLNX_PLAYBACK] ||
		...
		aud_drv_data->pdev =
			platform_device_register_resndata(&pdev->dev,
							  "z8_bone_island_wm8960", 0,	//Don't use xlnx_pl_snd_card. Made new machine driver that works with WM8960
							  //"xlnx_snd_card", 0,
							  NULL, 0,
							  &aud_drv_data->nodes,
							  pdata_size);
