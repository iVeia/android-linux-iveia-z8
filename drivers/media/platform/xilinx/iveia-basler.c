...
/*
 * Much of this is taken from the basler camera driver provided by 
 * basler for the NXP development kit.
 *
 * iVeia has taken that driver, removed the MIPI v4l2 node, and replaced
 * it with a dummy node.  This facilitates the processing of the MIPI data
 * in the PL as opposed to bringing it into the PS
 */
...
#define MODULE_NAME "iveia-basler-mipi"
#define MODULE_COMPAT "iveia,basler-mipi"
					   ...
					   struct basler_csi_information* bci)
{
  ...
  dev_warn(dev, "faking csi information\n");

  bci->max_lane_frequency = 1140000000 / 2;
  bci->lane_count = 4;
  for (i = 0; i < 4; ++i) {
    bci->lane_assignment[i] = i;
  }
  ret = 0;
  return ret;
}
...
static int basler_camera_get_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg, struct v4l2_subdev_format *format) {
  struct basler_camera_dev *bdev = to_basler_camera_dev(sd);
  format->format = bdev->format;

  pr_debug("Trying to get format to (%d x %d) %d %d %d %d %d\n",
           format->format.width,
...
{
  struct basler_camera_dev *bdev = to_basler_camera_dev(sd);
  pr_debug("Trying to set format to (%d x %d) %d %d %d %d %d\n",
           format->format.width,
           format->format.height,
           format->format.code,
           format->format.field,
			       ...
			       const struct i2c_device_id *id)
{
  ...
  if (ret) {
    dev_warn(dev, "Failed to get device information.  It won't work");
    dev_warn(dev, "Continuing to probe so that the media pipeline doesn't crash");
    sensor->found_i2c = 0;
  } else {
    sensor->found_i2c = 1;
  }
  ...
  dev_warn(dev, "media_entity_pads_init: %d\n", ret);
  if (ret)
    return ret;
...
MODULE_DESCRIPTION("Basler camera v4l2 dummy driver");
