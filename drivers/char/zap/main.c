...
ssize_t zap_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	#if defined(CONFIG_MACH_IV_ATLAS_I_Z7E) || defined(CONFIG_ARCH_IV_ATLAS_II_Z8)
...
void zap_cleanup_module(void)
{
	...
	unregister_chrdev_region(IV_ZAP_DEVNO, ZAP_MAX_DEVICES * 2);
}
...
int zap_init_module(void)
{
	...
	err = cdev_add (&zap_devp->cdev, IV_ZAP_DEVNO, ZAP_MAX_DEVICES * 2);
	if (err) {
